...
LOAD_NOISE_SCALE_FACTORS=("0" "1" "5")
DISP_NOISE_SCALE_FACTORS=("0" "0.5" "1" "2" "5")

# LOAD_NOISE_SCALE_FACTORS=("1") # testing
# DISP_NOISE_SCALE_FACTORS=("1")
...
for DISP_NOISE_SCALE_FACTOR in "${DISP_NOISE_SCALE_FACTORS[@]}"; do
  ...
  mpirun -n ${NUM_PROC} perturber ${DMG_FILE} "../../${PROBLEM}_synthetic/" ${TIME_STEPS} ${RANDOM_NOISE_SEED} ${DISP_NOISE_FACTOR} "$NOISY_SMB_NAME/"
...
python3 $ADD_NOISE_TO_LOAD "../load.dat" ${LOAD_EPS_STAR_NOISE} "../load_noise_scale_factors.txt" ${RANDOM_NOISE_SEED} ${NOISY_DIR_PREFIX}
cd ../..
    ...
    for PROBLEM_TYPE in "${PROBLEM_TYPES[@]}"; do
      ...
      if [ "$CALIBRATE"=true ]; then
        ...
        if (( $(echo "$mean_Y == 0" | bc -l) || $(echo "$mean_S == 0" | bc -l) || $(echo "$mean_D == 0" | bc -l) )); then
          ...
          mpirun -n ${NUM_PROC} primal "temp_${PROBLEM}.yaml" ##2>&1 | tee "calibration_${LOAD_NOISE_DIR}_${NOISY_DISP_DIR}_${PROBLEM}.out" '> uncomment for writing to the files'
